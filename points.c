...
int TIME_STEP = 0.0;
float ringMin = 0.0; float ringMax = 200.0;
...
typedef struct Particles {
  ...
  // Position/direction
  float pos[3];//pos of x,y,z, but it's 3 because it's a triangle
...
void moveParticle(Particle* p, GLfloat ang){
  ...
  } else if(p->pos[1] <= R*(amount + 0.05)) {
    p->pos[1] = R*(amount + 0.05);
  }//if
  p->vel[1]+=accel;
  //position updates
  p->pos[0] += p->vel[0]*cos(a);
  p->pos[2] += p->vel[2]*sin(a);
  p->pos[1] += p->vel[1];
}//moveParticle
...
void display(){
  ...
  // drawString(GLUT_BITMAP_HELVETICA_18, width/2, height/2, oxy_header);
  // drawString(GLUT_BITMAP_HELVETICA_18, width/2, 100 + height/2, ox_rate);
  drawParticles(firstParticles);
  //drawParticles(secondParticles);
  glutSwapBuffers();
}//display
